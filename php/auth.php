...
if ($data_login[0] == 3) 
{
    $query_auth = "SELECT `id_student`, students.name, roles.name, students.image FROM `students`, `roles` WHERE students.id_role = roles.id_role AND students.id_user = '$data_login[1]';";
    ...
    $_SESSION['you_avatar'] = "$data_auth[3]";
    header('Location: ../student_you.php');
}
...
if ($data_login[0] == 2) 
{
    $query_auth = "SELECT `id_teacher`, teachers.name, roles.name, teachers.image FROM `teachers`, `roles` WHERE teachers.id_role = roles.id_role AND teachers.id_user = '$data_login[1]';";
    $result_auth = mysqli_query($link, $query_auth);
    $data_auth = mysqli_fetch_row($result_auth);
    $_SESSION['id_user'] = "$data_auth[0]";
    $_SESSION['id_role'] = "$data_login[0]";
...
if ($data_login[0] == 1) 
{
    $query_auth = "SELECT `id_administration`, administrations.name, roles.name, administrations.image FROM `administrations`, `roles` WHERE administrations.id_role = roles.id_role AND administrations.id_user = '$data_login[1]';";
    $result_auth = mysqli_query($link, $query_auth);
    $data_auth = mysqli_fetch_row($result_auth);
    $_SESSION['id_user'] = "$data_auth[0]";
    $_SESSION['id_role'] = "$data_login[0]";
